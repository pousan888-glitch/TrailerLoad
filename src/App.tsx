    ...
    try {
      ...
      for (let i = 0; i < trailersList.length; i++) {
        ...
        const canvas = await html2canvas(element, {
          ...
          onclone: (clonedDoc) => {
            // CRITICAL FIX: Strip modern 'oklch' and 'oklab' color functions that break html2canvas/jsPDF
            const allElements = clonedDoc.getElementsByTagName('*');
            for (let j = 0; j < allElements.length; j++) {
              const el = allElements[j] as HTMLElement;
              const style = window.getComputedStyle(el);
              
              const hasOklab = (val: string) => val.includes('oklch') || val.includes('oklab');
              
              if (hasOklab(style.color)) el.style.color = '#1e293b';
              if (hasOklab(style.backgroundColor)) el.style.backgroundColor = '#ffffff';
              if (hasOklab(style.borderColor)) el.style.borderColor = '#e2e8f0';
              if (hasOklab(style.boxShadow)) el.style.boxShadow = 'none';
            }
          }
        });
        ...
        pdf.setFontSize(9);
        pdf.text(`Loading Plan - Page ${i + 1}/${trailersList.length} | Date: ${new Date().toLocaleDateString()}`, 15, 17);
        pdf.addImage(imgData, 'JPEG', 10, 22, 277, (canvas.height * 277) / canvas.width);
      }
